...
- Enforce lane isolation hard gate: team-role development edits are forbidden on `v1`/`koad-os` branches and must run only from dedicated lane branches (`lane/<ROLE>/<task-slug>`) in dedicated worktrees.
